...
В библиотеке есть каталог профилей поведения курсора `PERSONAS`. Для создания курсора с учётом выбранной персоны используйте фабрику `createPersonaCursor`.

Пример запуска отладочного скрипта с указанием профиля:

```bash
PERSONA=P3 npm run debug
```

Вы можете добавлять собственные профили, расширяя каталог в `src/personas.ts`.
