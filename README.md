...
Ghost Cursor включает набор профилей `PERSONAS`, задающих параметры «человеческого» поведения курсора. Для удобства можно воспользоваться фабрикой `createPersonaCursor(page, personaId, sessionId?)`, которая подставит параметры выбранной персоны.

```sh
PERSONA=P3 npm run debug
```

Такой запуск выберет профиль `P3`. Вы можете дополнять каталог персон в `src/personas.ts`.
